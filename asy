...
# SLOTS helper function (moved outside handle_button)
...
async def start_dice_match_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Initialize or retrieve user's main balance
    ...
    msg = (
        ...
        f"Bet: ₹{bet_amount:.2f}\n"
        f"Mode: 2 Dices, First to 2 Rounds (2d2w)\n" # Assuming mode is always 2d2w for now
        f"Current Balance: ₹{current_balance:.2f}\n\n"
        f"First roller is <b>{first_roller_label}</b>."
    )

    buttons = [
        [InlineKeyboardButton(f"Roll First Round 🎲", callback_data=f"dice_match_roll_round")],
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"CallbackQuery data: {query.data}") # Debug: See what callback is received
    ...
    if query.data == "climber_cashout":
        ...
        if climber_state.get("first_climb", True):
            ...
            # For simplicity, if game didn't truly start (no climbs), then it's a refund or no change.
            # If bet was not deducted, no win/loss. If it was, it's already in `balance` and we effectively refund it.
    ...
    # ----------------- DICE MATCH GAME (UPDATED TO USE EMOJIS) -----------------
    if query.data == "dice_match_roll_round":
        print(f"Entering dice_match_roll_round for user {user_id}") # Debug
        dice_match_state = user_dice_match_state.setdefault(user_id, {})
        if dice_match_state.get("game_over", False):
            print(f"Game is over for user {user_id}. Not rolling.") # Debug
        ...
        print(f"User {user_id} Balance: {current_balance}, Bet: {bet_amount}") # Debug

        # Send dice for the user
        await context.bot.send_message(chat_id=chat_id, text=f"{username}'s rolls:")
        user_roll_msg1 = await context.bot.send_dice(chat_id=chat_id, emoji='🎲')
        print(f"User 1st die value: {user_roll_msg1.dice.value}") # Debug
        await asyncio.sleep(1.5) # Wait for animation
        user_roll_msg2 = await context.bot.send_dice(chat_id=chat_id, emoji='🎲')
        print(f"User 2nd die value: {user_roll_msg2.dice.value}") # Debug
        await asyncio.sleep(1.5) # Wait for animation

        user_score = user_roll_msg1.dice.value + user_roll_msg2.dice.value

        # Send dice for the bot
        await context.bot.send_message(chat_id=chat_id, text="Bot's rolls:")
        bot_roll_msg1 = await context.bot.send_dice(chat_id=chat_id, emoji='🎲')
        print(f"Bot 1st die value: {bot_roll_msg1.dice.value}") # Debug
        await asyncio.sleep(1.5) # Wait for animation
        bot_roll_msg2 = await context.bot.send_dice(chat_id=chat_id, emoji='🎲')
        print(f"Bot 2nd die value: {bot_roll_msg2.dice.value}") # Debug
        await asyncio.sleep(1.5) # Wait for animation

        bot_score = bot_roll_msg1.dice.value + bot_roll_msg2.dice.value
        print(f"Bot total score: {bot_score}") # Debug

        dice_match_state["last_user_roll"] = (user_roll_msg1.dice.value, user_roll_msg2.dice.value)
        dice_match_state["last_bot_roll"] = (bot_roll_msg1.dice.value, bot_roll_msg2.dice.value)
        ...
        print(f"Rounds won - User: {user_rounds}, Bot: {bot_rounds}") # Debug
        ...
        dice_match_state["balance"] = current_balance # Update game's running balance
        print(f"Game Over: {game_over}, New Balance: {current_balance}") # Debug
        ...
        msg = (
            ...
            f"Mode: 2 Dices, First to 2 Rounds (2d2w)\n"
            f"Current Balance: ₹{current_balance:.2f}\n\n"
            f"Your Total: <b>{user_score}</b>\n"
            f"Bot Total: <b>{bot_score}</b>\n"
        ...
        if game_over:
            ...
            buttons.append([InlineKeyboardButton("Roll Next Round 🎲", callback_data="dice_match_roll_round")])
            buttons.append([InlineKeyboardButton("Cashout (Current Round)", callback_data="dice_match_cashout_mid_game")]) # Mid-game cashout

        buttons.append([InlineKeyboardButton("⬅️ Back to Regular Games", callback_data="regular_games")])

        reply_markup = InlineKeyboardMarkup(buttons)
        print(f"Attempting to edit message. Message ID: {query.message.message_id}, Chat ID: {chat_id}") # Debug
        try:
            await query.edit_message_text(
                msg, reply_markup=reply_markup, parse_mode="HTML", disable_web_page_preview=True
            )
            print(f"Message edited successfully.") # Debug
        except Exception as e:
            print(f"Error editing message: {e}") # Debug: Catch potential errors
            await context.bot.send_message(chat_id=chat_id, text=f"An error occurred: {e}") # Inform user
    ...
    if query.data == "dice_match_cashout": # This is for initial cashout (before any rolls)
        ...
        # Refund the bet if no rounds were played
        ...
        # Clear dice_match_state for the user
    ...
    if query.data == "dice_match_cashout_mid_game": # Cashout during an ongoing match
        ...
        current_balance = dice_match_state.get("balance", 100.0) # Balance already has initial bet deducted

        # Calculate partial winnings based on rounds won. This is a simplified model.
        # For a "first to 2" game, if you're 1-0 up, you might get a partial payout.
        # If 0-1 down, you might lose part of your bet.
        # Let's simplify: if you have more rounds won, you get a partial win, otherwise you lose your bet.
        
        win_amount = 0
        result_message = ""

        if user_rounds > bot_rounds:
            win_amount = bet_amount * (1 + (user_rounds * 0.5)) # Example: +50% for each round won
            result_message = f"Cashed out early! You partially win ₹{win_amount:.2f}."
        elif bot_rounds > user_rounds:
            win_amount = bet_amount * 0.1 # Very small refund, or just loss
            result_message = f"Cashed out early! You lost most of your bet. Refunded ₹{win_amount:.2f}."
        else: # Draw in rounds or no rounds played
            win_amount = bet_amount # Refund original bet
    ...
    if query.data == "dice_match_new_game":
        # This will send the /dice command effectively starting a new game
        # For a cleaner flow, we can just call start_dice_match_command logic directly
        # or redirect to the main games menu. For simplicity, let's redirect.
        await query.message.edit_text("Starting a new Dice Match...") # Acknowledge
        await query.message.reply_text("Please use `/dice <mode> <amount>` to start a new match.", parse_mode="Markdown")
        # Or, redirect to regular games menu:
        # await query.message.edit_text("Starting a new Dice Match...", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back to Regular Games", callback_data="regular_games")]]))
        return
