...
def main():
    ...
    application.add_handler(CommandHandler("newtradeid", newtradeid))
...
async def newtradeid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    new_id = await generate_trade_id()
    await update.message.reply_text(f"✨ Your new Trade ID is: `{new_id}`")
    user_deals = [
        deal for deal in active_deals.values()
        if (deal.get('creator_id') == user_id or deal.get('buyer_id') == user_id or deal.get('seller_id') == user_id)
        and deal.get('status') not in ['completed', 'cancelled', 'funds_released', 'refunded']
    ]
    if user_deals:
        ids = "\n".join(f"• `{d['trade_id']}` ({d['description']})" for d in user_deals)
        await update.message.reply_text(
            f"Your active Trade IDs:\n{ids}\n\nUse `/tradeid` in group for context-specific ID."
        )
    else:
        await update.message.reply_text("You currently have no active deals.")
