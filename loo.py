...
(
    ...
    # ... and hundreds more states for various complex flows
) = range(100) # Use a large range to accommodate many states
...
async def get_user_profile(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_profiles:
        user_profiles[user_id] = {
            "username": context.bot.get_chat(user_id).username if context.bot.get_chat(user_id) else None,
            "first_name": context.bot.get_chat(user_id).first_name if context.bot.get_chat(user_id) else None,
            "last_name": context.bot.get_chat(user_id).last_name if context.bot.get_chat(user_id) else None,
            "balance": 0.0,
            "saved_addresses": {},
            "referrals": [],
            "total_escrows": 0,
...
async def deal_description_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def new_deal_type_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if deal_type == 'simple_p2p':
        ...
        return DEAL_DESCRIPTION # Re-use deal description state for now, then lead to group creation
        ...
        return DEAL_DESCRIPTION # Will lead to milestone setup after initial details
    else:
...
async def escrow_type_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_profile = await get_user_profile(user.id, context)
    user_username = user_profile['username']

    if not user_username:
        await query.edit_message_text("You need a Telegram username to use this feature. Please set a username in Telegram settings.")
        return

    await query.edit_message_text("Setting up your trading group. Please wait...")

    group_type = query.data.replace('escrow_', '')
    try:
        invite_link, chat_id = await asyncio.wait_for(create_escrow_group_pyrogram(user.id, context.bot.username, group_type, context), timeout=30)
        note = "⚠ Note: This link is for 2 members (plus bot) only—third parties (unless invited by admin) are not allowed to join."
        
        # Store group chat ID and link with a newly generated trade ID
        trade_id = await generate_trade_id()
        active_deals[trade_id] = {
            'trade_id': trade_id,
            'creator_id': user.id,
            ...
            'created_at': datetime.now(),
            'description': 'Group-based deal', # Default description for group deals
            'amount': 0.0,
            'conditions': 'N/A',
            'seller_id': None,
            'buyer_id': None,
        ...
        await query.message.reply_text(
            ...
            f"Deal ID: `{trade_id}`\n\n"
            "Join this escrow group and share the link with the other party.\n\n"
            f"{invite_link}\n\n"
            f"{note}\n\n"
...
def main():
    ...
    # --- Conversation Handlers (for multi-step inputs) ---
    # Deal Details (dd) Conversation
    deal_details_conv_handler = ConversationHandler(
        entry_points=[CommandHandler('dd', dd)],
        states={
            DEAL_DESCRIPTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, deal_description_handler)],
        ...
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)] # Generic cancel
    )
    ...
    new_deal_conv_handler = ConversationHandler(
        ...
        states={
            ...
            DEAL_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, deal_amount_handler)],
            DEAL_CONDITIONS: [MessageHandler(filters.TEXT & ~filters.COMMAND, deal_conditions_handler)],
            # More states would be added here for different deal types (e.g., milestone setup, OTC participant info)
        },
    ...
    dispute_conv_handler = ConversationHandler(
        ...
        states={
            ...
            DISPUTE_PROOF: [MessageHandler(filters.PHOTO | filters.DOCUMENT | (filters.TEXT & ~filters.COMMAND), dispute_proof_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )
    ...
    admin_schedule_post_conv_handler = ConversationHandler(
        ...
        states={
            ...
            CHANNEL_POST_CONTENT: [MessageHandler(filters.TEXT | filters.PHOTO | filters.DOCUMENT, admin_schedule_post_content_handler)],
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )
    ...
    otc_deal_conv_handler = ConversationHandler(
        ...
        states={
            OTC_PARTICIPANT_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, otc_participant_id_handler)],
            DEAL_DESCRIPTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, deal_description_handler)], # Re-use existing states
            DEAL_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, deal_amount_handler)],
            DEAL_CONDITIONS: [MessageHandler(filters.TEXT & ~filters.COMMAND, deal_conditions_handler)],
            # This flow would then lead to escrow group creation
        },
        fallbacks=[CommandHandler('cancel', lambda u, c: ConversationHandler.END)]
    )
